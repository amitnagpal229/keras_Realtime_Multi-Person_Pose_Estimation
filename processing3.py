...
def get_model_blob(input_image, params, model, model_params):
    ...
    for scale in multiplier:
        ...
        blob['model'][scale] = {'image_to_test_padded_shape': image_to_test_padded.shape, 'pad': pad}
        blob['model'][scale]['output_blobs'] = model.predict(input_img)

    return blob


def extract_parts(blob, params, model_params):
    input_image_shape = blob["input_image_shape"]

    # Body parts location heatmap, one per part (19)
    heatmap_avg = np.zeros((input_image_shape[0], input_image_shape[1], 19))
    # Part affinities, one per limb (38)
    paf_avg = np.zeros((input_image_shape[0], input_image_shape[1], 38))

    for scale in blob["model"]:
        output_blobs = blob["model"][scale]['output_blobs']
        image_to_test_padded_shape = blob["model"][scale]['image_to_test_padded_shape']
        pad = blob["model"][scale]['pad']

        # extract outputs, resize, and remove padding
        heatmap = np.squeeze(output_blobs[1])  # output 1 is heatmaps
        heatmap = cv2.resize(heatmap, (0, 0), fx=model_params['stride'], fy=model_params['stride'],
                             interpolation=cv2.INTER_CUBIC)
        heatmap = heatmap[:image_to_test_padded_shape[0] - pad[2], :image_to_test_padded_shape[1] - pad[3], :]
        heatmap = cv2.resize(heatmap, (input_image_shape[1], input_image_shape[0]), interpolation=cv2.INTER_CUBIC)

        paf = np.squeeze(output_blobs[0])  # output 0 is PAFs
        paf = cv2.resize(paf, (0, 0), fx=model_params['stride'], fy=model_params['stride'],
                         interpolation=cv2.INTER_CUBIC)
        paf = paf[:image_to_test_padded_shape[0] - pad[2], :image_to_test_padded_shape[1] - pad[3], :]
        paf = cv2.resize(paf, (input_image_shape[1], input_image_shape[0]), interpolation=cv2.INTER_CUBIC)

        heatmap_avg = heatmap_avg + heatmap / len(blob["model"])
        paf_avg = paf_avg + paf / len(blob["model"])
            ...
            for i in range(n_a):
                for j in range(n_b):
                    ...
                    score_with_dist_prior = sum(score_midpts) / len(score_midpts) + min(
                        0.5 * input_image_shape[0] / norm - 1, 0)
